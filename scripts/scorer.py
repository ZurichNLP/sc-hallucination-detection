...
def main(ref_dicts, pred_dicts, pred_file_path, output_file=None):
    ...
    if output_file is not None:
        with open(output_file, 'a') as ostr:  # Open in append mode
            print(f'Prediction File Path: {pred_file_path}', file=ostr)
            print(f'IoU: {ious.mean():.8f}', file=ostr)
            print(f'Cor: {cors.mean():.8f}', file=ostr)
            print("-" * 30, file=ostr)  # Add a separator for readability
    return ious, cors
...
if __name__ == '__main__':
    ...
    #p.add_argument('pred_folder', type=str)  # Changed to a folder instead of a single file
    p.add_argument('pred_file', type=str)
    ...
    """# Iterate through all prediction files in the specified folder
    for filename in os.listdir(a.pred_folder):
        pred_file_path = os.path.join(a.pred_folder, filename)
        
        # Skip non-prediction files if needed
        if filename.endswith(".errors.txt"):
            continue

        # Load each prediction file and process it
        pred_data = load_jsonl_file_to_records(pred_file_path)
        _ = main(ref_data, pred_data, pred_file_path, a.output_file)"""

    os.makedirs(os.path.dirname(a.output_file), exist_ok=True)
    pred_data = load_jsonl_file_to_records(a.pred_file)
    _ = main(ref_data, pred_data, a.pred_file, a.output_file)