...
parser.add_argument("data_split", type=str, help="data split to generate responses for. e.g.: test, val")
...
if args.data_split == 'test_jan25':
    file_path = f"data/{args.data_split}/mushroom.hi-tst.v1.jsonl"
else:
    file_path = f"data/v2_splits/{args.data_split}/mushroom.hi-val.v2.jsonl" # TODO: change according to needs
...
for shorthand, config in tqdm.tqdm(configs):
    output_file_path = os.path.join(f'./data/alt_res_{args.data_split}_setv2/alt_res/hi', f'hindi-{model_name.split("/")[1]}.{shorthand}.jsonl')
    os.makedirs(f'./data/alt_res_{args.data_split}_setv2/alt_res/hi', exist_ok=True)
