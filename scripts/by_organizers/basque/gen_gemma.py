...
args.add_argument("data_split", type=str, help="data split to generate responses for. e.g.: test, val, test_jan25")
...
if args.data_split == 'test_jan25':
    file_path = f"data/{args.data_split}/mushroom.eu-tst.v1.jsonl"
else:
    file_path = f"data/v2_splits/{args.data_split}/mushroom.eu-val.v2.jsonl" # TODO: change according to needs
...
for shorthand, config in tqdm.tqdm(configs):
    print(config)
    output_file_path = os.path.join(f'./data/alt_res_{args.data_split}_setv2/alt_res/eu', f'basque-{model_name.split("/")[1]}.{shorthand}.jsonl')
    os.makedirs(f'./data/alt_res_{args.data_split}_setv2/alt_res/eu', exist_ok=True)
