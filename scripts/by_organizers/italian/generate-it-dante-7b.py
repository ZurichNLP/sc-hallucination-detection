...
# MW: THIS MODEL IS NOT REPRESENTED IN THE DATASET
...
if args.data_split == 'test_jan25': 
    file_path = f"data/{args.data_split}/mushroom.it-tst.v1.jsonl"
else:
    file_path = f"data/v2_splits/{args.data_split}/mushroom.it-val.v2.jsonl" # TODO: change according to needs
...
def main():
    ...
    for shorthand, config in tqdm.tqdm(configs, desc='configs'):
        output_file_path = os.path.join(f'./data/alt_res_{args.data_split}_setv2/alt_res/it', f'italian-{model_name.split("/")[1]}.{shorthand}.jsonl')
        os.makedirs(f'./data/alt_res_{args.data_split}_setv2/alt_res/it', exist_ok=True)
        ...
        with open(output_file_path, 'w') as file:
            for record_ in tqdm.tqdm(records, desc='items'):
                ...
                for response_index in range(5):
                    # print("output:", output)
                    response_text = tokenizer.decode(output.sequences[response_index], skip_special_tokens=False)
                    ...
                    response_logits = [l[response_index, token_id].item() for l, token_id in zip(output.logits, response_token_ids)]
