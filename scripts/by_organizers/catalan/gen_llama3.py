...
if args.data_split == 'test_jan25':     
    file_path = f"data/{args.data_split}/mushroom.ca-tst.v1.jsonl"
else:
    file_path = f"data/v2_splits/{args.data_split}/mushroom.ca-val.v2.jsonl" # TODO: change according to needs
...
for shorthand, config in tqdm.tqdm(configs):
    output_file_path = os.path.join(f'./data/alt_res_{args.data_split}_setv2/alt_res/ca', f'catalan-{model_name.split("/")[1]}.{shorthand}.jsonl')
    os.makedirs(f'./data/alt_res_{args.data_split}_setv2/alt_res/ca', exist_ok=True)
