...
if args.data_split == 'test_jan25':
    file_path = f"data/{args.data_split}/mushroom.en-tst.v1.jsonl"
else:
    file_path = f"data/v2_splits/{args.data_split}/mushroom.en-val.v2.jsonl" 
...
for shorthand, config in tqdm.tqdm(configs, desc='configs'):
    output_file_path = os.path.join(f'./data/alt_res_{args.data_split}_setv2/alt_res/en', f'english-{model_name_or_path.split("/")[1]}.{shorthand}.jsonl')
    os.makedirs(f'./data/alt_res_{args.data_split}_setv2/alt_res/en', exist_ok=True)
    ...
    with open(output_file_path, 'w') as file:
        for record in tqdm.tqdm(records, desc='items'):
            ...
            # Fix: Convert inputs to the correct device and get the input_ids tensor directly
            ...
            outputs = model.generate(
                    input_ids,  # Changed from inputs to input_ids
                    max_new_tokens=512,
            ...
            for response_index in range(5):
                # Fix: Use input_ids.shape instead of inputs.shape
