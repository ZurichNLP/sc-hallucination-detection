...
class SentenceAligner(object):
	...
	def get_word_aligns_with_scores(self, src_sent: Union[str, List[str]], trg_sent: Union[str, List[str]]) -> Dict[str, Union[List[Tuple[int, int]], List[Tuple[int, int, float]]]]:
			...
			for j in range(len(vectors[1])):
				for ext in self.matching_methods:
					if all_mats[ext][i, j] > 0:
						if self.token_type == "bpe":
							aligns[ext].add(((l1_b2w_map[i], l2_b2w_map[j]), sim[i, j]))
						else:
							aligns[ext].add((i, j))
		...
		for ext in aligns:
			...
			aligned_idcs = [tup[0] for tup, score in aligns[ext]]
			total_idcs = [idx for idx, sublist in enumerate(l1_tokens) for _ in range(len(sublist))]
			for aligned_index in aligned_idcs:
				total_idcs.pop(total_idcs.index(aligned_index))
			for t_idcs in total_idcs:
				aligns[ext].append(((t_idcs, -1), 0.0))
			aligns[ext] = sorted(aligns[ext])
